...
\captionof{figure}{These phase contrast image and phase contrast cross section were obtained using my non-Fourier method. As it can be seen here the phase contrast peaks are much higher than the profile found using the Fourier method. The parameters used to obtain this image were an X-ray energy equal to $24.942$keV (corresponding to a Ag k-beta1 source), the cylinder's material was water which has a refractive index decrement $\delta_1 = 369.763$ nm while the attenuation coefficient at this energy is $\mu_1 = 50.9387\mathrm{m^{-1}}$, the sigmoid function blurred 0.14 pixels over the edge of the imaged cylinder. The apparent asymmetry of the two-dimensional phase contrast image is due to aliasing.}
\end{Figure}
...
% Therefore my first simulation will be that of a single monomorphic cylinder, and later on I will work on 2 cylinders, one embedded on another, both these cylinders will have different densities, since I want to understand how much density difference is needed before I see phase contrast in my simulation.
